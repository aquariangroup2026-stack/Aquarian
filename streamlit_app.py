...
if df is not None:
    ...
    with tabs2:
        ...
        train_df = df.iloc[:-31]  # Old data for training
        val_df = df.iloc[-31:]    # Newest 31 days for testing

        if val_df.empty:
            st.warning("Not enough data to run validation.")
        else:
            # 2. Retrain a temporary model on PAST data only (Prevents cheating)
            # We use the same 'features' list from your data loader
            val_model = LogisticRegression(solver='liblinear')
            val_model.fit(train_df[features], train_df['Rain_Binary'])

            # 3. Predict on the hidden 31 days
            val_probs = val_model.predict_proba(val_df[features])[:, 1]
            
            # 4. Plotting (Your updated logic)
            st.subheader("Predicted vs Actual (Last 31 Days)")
            fig3, ax3 = plt.subplots(figsize=(12, 5))
            
            # Actual Rain = Red Bars (Height 1.0)
            # We ensure bars are visible by setting a slight alpha
            ax3.bar(val_df['Date'], val_df['Rain_Binary'], color='red', alpha=0.3, label='Actual Rain (Yes/No)')
            
            # Engine Probability = Blue Line
            ax3.plot(val_df['Date'], val_probs, color='blue', marker='o', linewidth=2, label='Engine Probability')
            
            # Formatting
            ax3.axhline(0.5, color='gray', linestyle='--', alpha=0.5, label='Decision Threshold')
            ax3.set_ylabel("Probability (0-1)")
            ax3.set_title("Did the model catch the rain events?")
            ax3.legend(loc='upper left')
            
            st.pyplot(fig3)

            # 5. Quick Accuracy Score
            from sklearn.metrics import accuracy_score
            acc = accuracy_score(val_df['Rain_Binary'], (val_probs > 0.5).astype(int))
            st.caption(f"Model accuracy on these unseen days: **{acc:.1%}**")
